...
func TestContextCancellationStopsGoroutines(t *testing.T) {
	...
	// Get the number of goroutines before cancellation
	...
	// Get the number of goroutines after cancellation
	afterGoroutines := runtime.NumGoroutine()

	// Verify that the number of goroutines decreased
	if afterGoroutines >= beforeGoroutines {
		t.Errorf("Expected number of goroutines to decrease after context cancellation. Before: %d, After: %d", beforeGoroutines, afterGoroutines)
	}

	// Verify that the DHT's context is done
	select {
	case <-dht.Context().Done():
		// Context is done as expected
	default:
		t.Error("Expected DHT's context to be done after cancellation")
	}

	// Verify that all DHT-related goroutines have stopped
	// 1. Check that rtPeerLoop has stopped
	select {
	case <-dht.addPeerToRTChan:
		t.Error("Expected addPeerToRTChan to be closed after cancellation")
	default:
		// Channel closed, goroutine stopped
	}

	// 2. Check that runFixLowPeersLoop has stopped
	select {
	case <-dht.fixLowPeersChan:
		t.Error("Expected fixLowPeersChan to be closed after cancellation")
	default:
		// Channel closed, goroutine stopped
	}
}
