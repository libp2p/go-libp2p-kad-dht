...
func TestStats(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		require.Equal(t, int64(0), stats.Queues.PendingKeyProvides)
		require.Equal(t, int64(0), stats.Queues.PendingRegionProvides)
		require.Equal(t, int64(0), stats.Queues.PendingRegionReprovides)
		require.Equal(t, int64(0), stats.Schedule.Keys)
		require.Equal(t, int64(0), stats.Schedule.Regions)
		...
		require.Equal(t, int64(0), stats.Operations.Past.RecordsProvided)
		require.Equal(t, int64(0), stats.Operations.Past.KeysProvided)
		require.Equal(t, int64(0), stats.Operations.Past.KeysFailed)
		...
		require.Equal(t, int64(0), stats.Operations.Past.RegionReprovidedLastCycle)
		...
		require.Equal(t, int64(0), stats.Queues.PendingRegionProvides)
		require.Equal(t, int64(0), stats.Queues.PendingRegionReprovides)
		require.Equal(t, int64(1), stats.Schedule.Keys)
		require.Equal(t, int64(1), stats.Schedule.Regions)
		require.Equal(t, avgPrefixLenFloat, stats.Schedule.AvgPrefixLength)
		...
		require.Equal(t, 0, stats.Operations.Ongoing.KeyReprovides)
		require.Equal(t, 0, stats.Operations.Ongoing.RegionReprovides)
		require.Equal(t, int64(recordsProvided), stats.Operations.Past.RecordsProvided)
		require.Equal(t, int64(1), stats.Operations.Past.KeysProvided)
		require.Equal(t, int64(0), stats.Operations.Past.KeysFailed)
		...
		require.Equal(t, int64(0), stats.Queues.PendingRegionProvides)
		require.Equal(t, int64(0), stats.Queues.PendingRegionReprovides)
		require.Equal(t, int64(reprovidedKeys), stats.Schedule.Keys)
		require.Equal(t, int64(0), stats.Schedule.Regions) // during the reprovide region is removed from schedule, it will be added back after reprovide is complete
		require.Equal(t, avgPrefixLenFloat, stats.Schedule.AvgPrefixLength)
		require.Equal(t, bitstr.Key(strings.Repeat("0", avgPrefixLen)), stats.Schedule.NextReprovidePrefix)
		...
		require.Equal(t, 0, stats.Operations.Ongoing.KeyReprovides)
		require.Equal(t, 0, stats.Operations.Ongoing.RegionReprovides)
		require.Equal(t, int64(newKeysProvidedRecords+2*recordsProvided), stats.Operations.Past.RecordsProvided)
		require.Equal(t, int64(reprovidedKeys+1), stats.Operations.Past.KeysProvided) // 1 provided, 6 reprovided
		require.Equal(t, int64(0), stats.Operations.Past.KeysFailed)
		require.Equal(t, 1., stats.Operations.Past.KeysProvidedPerMinute)
		...
		require.Equal(t, int64(1), stats.Operations.Past.RegionReprovidedLastCycle)
		...
		require.Equal(t, "online", stats.Connectivity.Status)
		require.Equal(t, startTime, stats.Connectivity.Since)
		require.Equal(t, int64(len(balancedKeys)-keysPerPrefix*concurrentProvides), stats.Queues.PendingKeyProvides)    // exactly 3 keys per region
		require.Equal(t, int64(len(balancedKeys)/keysPerPrefix-concurrentProvides), stats.Queues.PendingRegionProvides) // exactly 3 keys per region
		require.Equal(t, int64(0), stats.Queues.PendingRegionReprovides)
		require.Equal(t, int64(reprovidedKeys+len(balancedKeys)), stats.Schedule.Keys)
		require.Equal(t, int64(nRegions), stats.Schedule.Regions)
		require.Equal(t, avgPrefixLenFloat, stats.Schedule.AvgPrefixLength)
		...
		require.Equal(t, 0, stats.Operations.Ongoing.KeyReprovides)
		require.Equal(t, 0, stats.Operations.Ongoing.RegionReprovides)
		require.Equal(t, int64(balancedKeysRecords+newKeysProvidedRecords+2*recordsProvided), stats.Operations.Past.RecordsProvided)
		require.Equal(t, int64(len(balancedKeys)+reprovidedKeys+1), stats.Operations.Past.KeysProvided)
		require.Equal(t, int64(0), stats.Operations.Past.KeysFailed)
		require.Equal(t, 0., stats.Operations.Past.KeysProvidedPerMinute) // in the last cycle (last reprovideInterval), we didn't provide any new key
		require.Equal(t, float64(reprovidedKeys)/float64(reprovideDuration.Minutes()), stats.Operations.Past.KeysReprovidedPerMinute)
		require.Equal(t, reprovideDuration, stats.Operations.Past.RegionReprovideDuration)
		...
		require.Equal(t, 0, stats.Operations.Ongoing.KeyReprovides)
		require.Equal(t, 0, stats.Operations.Ongoing.RegionReprovides)
		require.Equal(t, int64(2*(balancedKeysRecords+newKeysProvidedRecords+recordsProvided)+recordsProvided), stats.Operations.Past.RecordsProvided)
		require.Equal(t, int64(2*(len(balancedKeys)+reprovidedKeys)+1), stats.Operations.Past.KeysProvided)
		require.Equal(t, int64(0), stats.Operations.Past.KeysFailed)
		require.Equal(t, 0., stats.Operations.Past.KeysProvidedPerMinute) // in the last cycle (last reprovideInterval), we didn't provide any new key
		require.Equal(t, 0., stats.Operations.Past.KeysReprovidedPerMinute)
		...
		require.Equal(t, int64(nRegions), stats.Operations.Past.RegionReprovidedLastCycle)
