...
func (p *dhtQueryPath) queryPeer(proc process.Process, pid peer.ID) {
	...
	if err != nil {
		logger.Debugf("ERROR worker for: %v %v", pid, err)
		r.Lock()
		r.errs = append(r.errs, err)
		r.Unlock()

	} else if res.success {
		logger.Debugf("SUCCESS worker for: %v %s", pid, res)
		r.Lock()
		p.result = res
		r.Unlock()
		...
		logger.Debugf("PEERS CLOSER -- worker for: %v (%d closer peers)", pid, len(res.closerPeers))
		for _, next := range res.closerPeers {
			if next.ID == r.query.dht.self { // don't add self.
				logger.Debugf("PEERS CLOSER -- worker for: %v found self", pid)
				continue
			}
			...
			logger.Debugf("PEERS CLOSER -- worker for: %v added %v (%v)", pid, next.ID, next.Addrs)
		}
	} else {
		logger.Debugf("QUERY worker for: %v - not found, and no closer peers.", pid)
	}
}
