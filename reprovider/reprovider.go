...
func (s *SweepingReprovider) addCids(cids []mh.Multihash) (map[bitstr.Key][]mh.Multihash, int, error) {
	prefixes := make(map[bitstr.Key][]mh.Multihash)
	avgPrefixLen := 0

	cids, err := s.mhStore.Put(s.ctx, cids...)
	if err != nil {
		return nil, 0, err
	}
